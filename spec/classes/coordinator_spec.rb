...
describe 'druid::coordinator', :type => 'class' do
  ...
  context 'On base system with custom JVM parameters ' do
    let(:params) do
      ...
      it {
        should contain_file('/etc/systemd/system/druid-coordinator.service')\
          .with_content("[Unit]\nDescription=Druid Coordinator Node\n\n[Service]\nType=simple\nWorkingDirectory=/etc/druid/coordinator/\nExecStart=/usr/bin/java -server -Xmx25g -Xms25g -XX:NewSize=6g -XX:MaxNewSize=6g -XX:MaxDirectMemorySize=64g -Duser.timezone=PDT -Dfile.encoding=latin-1 -Djava.util.logging.manager=custom.LogManager -Djava.io.tmpdir=/mnt/tmp -classpath .:/usr/local/lib/druid/lib/* org.apache.druid.cli.Main server coordinator\nSuccessExitStatus=130 143\nRestart=on-failure\n\n[Install]\nWantedBy=multi-user.target\n")
      }
  end
