...
describe 'druid::indexing::middle_manager', :type => 'class' do
  ...
  context 'On base system with custom JVM parameters ' do
    ...
    it {
      should contain_file('/etc/systemd/system/druid-middle_manager.service').with_content("[Unit]\nDescription=Druid Middle Manager Node\n\n[Service]\nType=simple\nWorkingDirectory=/etc/druid/middle_manager/\nExecStart=/usr/bin/java -server -Xmx4g -Xms4g -XX:NewSize=256m -XX:MaxNewSize=256m -XX:MaxDirectMemorySize=2g -Duser.timezone=PDT -Dfile.encoding=latin-1 -Djava.util.logging.manager=custom.LogManager -Djava.io.tmpdir=/mnt/tmp -classpath .:/usr/local/lib/druid/lib/* org.apache.druid.cli.Main server middleManager\nSuccessExitStatus=130 143\nRestart=on-failure\n\n[Install]\nWantedBy=multi-user.target\n")
    }
  end
    ...
    let(:params) do
      {
        ...
        :runner_allowed_prefixes         => ['druid', 'org.apache.druid', 'user.timezone', 'file.encoding'],
